...
const p: Permissions = { policy: { precedence: "deny_over_allow" }, filesystem: { edit: "allow", write: "allow" }, shell: { default: "ask", allow: ["git*", "pnpm*"], deny: ["git push*", "git reset --hard*", "git clean*", "git rm*", "gh pr create*", "gh api*"] } };
...
test("glob conversion and generated hook block only dangerous Bash commands", () => {
  ...
  const dir = mkdtempSync(join(tmpdir(), "agentctl-test-")); const hook = join(dir, "policy.py");
  ...
  for (const command of denied) {
    const run = spawnSync("python3", [hook], { input: JSON.stringify({ tool_name: "Bash", tool_input: { command } }), encoding: "utf8" });
