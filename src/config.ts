...
async function yamlFile(path: string): Promise<unknown> {
  ...
  catch (error) { throw new Error(`Cannot parse ${path}: ${(error as Error).message}`); }
}

export async function loadSource(root: string): Promise<{ config: AgentctlConfig; permissions: Permissions }> {
