...
interface GeneratedFile { path: string; content: string; executable?: boolean; runtime: string }
...
function expected(root: string, source: Awaited<ReturnType<typeof loadSource>>): GeneratedFile[] {
  ...
  if (source.config.runtimes.claude.enabled) files.push({ runtime: "Claude", path: resolve(root, ".claude/settings.json"), content: renderClaude(source.permissions) });
  if (source.config.runtimes.opencode.enabled) files.push({ runtime: "OpenCode", path: resolve(root, ".opencode/opencode.json"), content: renderOpenCode(source.permissions) });
  if (source.config.runtimes.codex.enabled) {
    files.push({ runtime: "Codex", path: resolve(root, ".codex/config.toml"), content: renderCodexConfig(source.permissions) });
    files.push({ runtime: "Codex", path: resolve(root, ".codex/hooks/permission-policy.py"), content: renderCodexHook(source.permissions), executable: true });
  }
  return files;
}
...
function display(root: string, path: string): string { return relative(root, path) || path; }
...
async function main(): Promise<void> {
  ...
  if (!command || !["sync", "check", "validate", "diff"].includes(command)) {
    console.error("Usage: agentctl <sync|check|validate|diff>"); process.exitCode = 2; return;
  }
  const root = process.cwd();
  let source: Awaited<ReturnType<typeof loadSource>>;
  try { source = await loadSource(root); }
  catch (error) { console.error(`❌ Validation failed: ${(error as Error).message}`); process.exitCode = 1; return; }
  if (command === "validate") { console.log("✅ .ai configuration is valid."); return; }
  ...
  if (command === "sync") {
    ...
    console.log("\n✅ Agent configuration sync complete."); return;
  }
  let drift = false;
  if (command === "check") console.log("🔍 Checking agent configurations...\n");
  for (const file of files) {
    const before = await current(file.path);
    const differs = before !== file.content || (file.executable && (!existsSync(file.path) || ((await stat(file.path)).mode & 0o111) === 0));
    if (differs) {
      drift = true;
